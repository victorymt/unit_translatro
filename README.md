...
- 工作台实时计算倍率、账号成本、用户自有 1 亿混合 Token 的实际支出，并在同一页突出 ChatGPT 中转与所有配置渠道的 CNY 和相对成本；`c` 渠道页会显示固定的 ChatGPT 中转基准、完整 USD 单价，以及与主屏当前模式、用量和汇率同步的 CNY 成本与相对倍数。宽屏成本列默认压缩到 4 位小数，选中详情保留完整精度。充值比例、ChatGPT 输入/输出/缓存价和美元汇率在主屏编辑，输入/输出/缓存用量配比通过 `u` 调整。
- 渠道管理可新建、编辑和删除名称、提供商、模型、三类单价、生效日期、来源和版本。渠道单价固定为 `USD / 1M tokens`；ChatGPT 中转基准只读显示，不在可编辑渠道列表中重复维护。

默认按 `1 元 = 1 刀站内额度`、ChatGPT 输入 `5 刀`、输出 `30 刀`、缓存 `0.5 刀`、`1 USD = 7.2 CNY` 计算。主屏以 `80 x 24` 或更大的窗口获得完整布局；`72 x 20` 以上的窄窗口会保留输入和结果并折叠渠道行，低于 `72 x 20` 才会提示放大窗口。

主屏使用 `Tab`/`Shift+Tab` 或上下方向键切换输入框，左右方向键编辑当前数字，`m` 切换换算模式，`u` 编辑高级 Token 用量配比，`s` 保存，`r` 还原，`c` 管理渠道，`q` 退出；按 `F1` 或 `?` 可打开当前页面的快捷键帮助。主屏参数区采用左右两列三行紧凑布局。底部常驻“快速计算”面板按 `F6` 聚焦/释放、`F7` 折叠/展开：输入单行 `bc` 算式后按 Enter 计算，历史以“算式/结果”两行堆叠并紧贴输入框上方，面板会按终端剩余高度显示最近记录；↑/↓ 仍可复用本次运行最近 20 条历史，Esc 退出计算器焦点。成功计算后输入框清空并保持焦点，错误会保留原算式以便修正；历史包含成功和错误记录，不写入配置文件。面板在主屏、渠道页和用量页都可使用；用量及渠道编辑表单也可用 `F6` 切换焦点，聚焦时底部会明确提示如何释放焦点。固定 1 亿实际支出模式会把该配比归一化到 1 亿 Token；其他模式的结果标为“当前用量成本”并按实际总量计算。渠道页使用 `n` 新建、`N` 复制当前渠道、`e` 编辑和 `d` 删除，`/` 按名称、提供商或模型筛选（再次按 `/` 并清空可恢复全部），`Home`/`End` 可跳到首尾；删除渠道始终要求确认，存在未保存修改时退出和还原也会要求确认。主屏在窄窗口中会折叠渠道对比并保留“按 `c` 查看全部”的提示。

快速计算器使用系统的 `bc -l`（PTY 常驻进程）以保留 `scale`、变量和函数状态；输入限制为单行 ASCII 文本并带超时保护。未安装 `bc` 或计算超时时，面板会显示错误，主 TUI 仍可继续使用。`bc` 是可选运行时工具，不会作为 Python 依赖安装。
