...
主屏使用 `Tab`/`Shift+Tab` 或上下方向键切换输入框，左右方向键编辑当前数字，`m` 切换换算模式，`u` 编辑高级 Token 用量配比，`s` 保存，`r` 还原，`c` 管理渠道，`q` 退出。主屏参数区采用左右两列三行紧凑布局。底部常驻“快速计算”面板按 `F6` 聚焦/释放：输入单行 `bc` 算式后按 Enter 计算，历史以“算式/结果”两行堆叠并紧贴输入框上方，面板会按终端剩余高度显示最近记录；↑/↓ 仍可复用本次运行最近 20 条历史，Esc 退出计算器焦点。成功计算后输入框清空并保持焦点，错误会保留原算式以便修正；历史包含成功和错误记录，不写入配置文件。面板在主屏、渠道页和用量页都可使用；用量及渠道编辑表单也可用 `F6` 切换焦点。固定 1 亿实际支出模式会把该配比归一化到 1 亿 Token；其他模式的结果标为“当前用量成本”并按实际总量计算。渠道页使用 `n`/`e`/`d` 新建、编辑和删除；删除渠道始终要求确认，存在未保存修改时退出和还原也会要求确认。主屏在窄窗口中会折叠渠道对比并保留“按 `c` 查看全部”的提示。
