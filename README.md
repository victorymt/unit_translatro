...
也可以使用 `uv run python unit_converter.py` 直接运行源码。这是一个 Textual 工作台，有“换算”、“价格对比”和“渠道管理”三个视图：

- 换算页实时计算倍率、账号成本、ChatGPT 中转的 1 亿混合 Token 成本。充值比例、ChatGPT 输入/输出/缓存价和美元汇率在这里编辑。
- 价格对比页实时展示 ChatGPT 中转与所有配置渠道的 USD、CNY 和相对成本，可在窄终端直接查看完整表格。
