...
- 工作台实时计算倍率、账号成本、用户自有 1 亿混合 Token 的实际支出，并在同一页突出 ChatGPT 中转与所有配置渠道的 CNY 和相对成本；`c` 渠道页会显示固定的 ChatGPT 中转基准、完整 USD 单价，以及与主屏当前模式和用量同步的 CNY 成本与相对倍数。充值比例、ChatGPT 输入/输出/缓存价和美元汇率在主屏编辑，输入/输出/缓存用量配比通过 `u` 调整。
