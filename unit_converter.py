...
class TuiState:
    ...
    def edit(self, key: int) -> None:
        field = "value" if self.active_field == 0 else "ratio"
...
def _result_for(state: TuiState) -> tuple[str, str, str]:
    if not state.value or not state.ratio:
        return "--", "", ""
    try:
...
def _draw_tui(screen: curses.window, state: TuiState) -> None:
    ...
    _addstr(screen, 10, left + 18, value_field, value_attr)
    _addstr(screen, 10, left + 18 + _display_width(value_field) + 1, unit)
    ...
    _addstr(screen, 12, left + 18, ratio_field, ratio_attr)
    _addstr(screen, 12, left + 18 + _display_width(ratio_field) + 1, "刀额度")

    result, secondary, error = _result_for(state)
    result_label = "换算结果"
    _addstr(screen, 14, left + 2, result_label, curses.A_DIM)
    _addstr(screen, 14, left + 18, result, curses.A_BOLD | success)
    if secondary:
        _addstr(screen, 15, left + 18, secondary, curses.A_DIM)
    if error:
        _addstr(screen, 16, left + 2, error, error_color)

    screen.refresh()


def _curses_main(screen: curses.window) -> tuple[str, str] | None:
    state = TuiState()
    curses.curs_set(0)
    screen.keypad(True)
